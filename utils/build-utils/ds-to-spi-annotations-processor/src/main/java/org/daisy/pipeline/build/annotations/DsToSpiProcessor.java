...
public class DsToSpiProcessor extends AbstractProcessor {
			...
			for (Element e : roundEnv.getElementsAnnotatedWith(Component.class)) {
				if (e.getKind() == ElementKind.CLASS) {
					...
					ComponentModel component = new ComponentModel(); {
						component.packageName = packageElement.getQualifiedName().toString();
						if (component.packageName.equals("")) {
							component.packageName = null;
						}
						component.className = component.packageName != null ?
							classElement.getQualifiedName().toString().substring(component.packageName.length() + 1) :
							classElement.getQualifiedName().toString();
						component.qualifiedClassName = component.packageName != null ?
							component.packageName + "." + component.className :
							component.className;
						boolean include = false; {
							String fqn = (component.packageName != null ? component.packageName + "." : "")
								+ component.className.replaceAll("\\.", "\\$");
						...
						if (include) {
							...
							if (serviceList != null) {
								for (AnnotationValue v : (List<? extends AnnotationValue>)serviceList.getValue()) {
									String service = v.getValue().toString();
							...
							excludeClasses.add(classElement.getQualifiedName().toString());
						}
					}
						...
						ActivateModel activate = new ActivateModel(); {
							...
							if (exeElement.getParameters().size() == 0) {
								...
								try {
									...
									if (Dictionary.class.isAssignableFrom(argType) || Map.class.isAssignableFrom(argType)) {
										printWarning("No component configuration with SPI except within component declaration itself", e);
										activate.propertiesArgumentType = argType;
									} else {
	...
	private AnnotationValue getAnnotationValue(Element element, String annotationName, String valueName) {
