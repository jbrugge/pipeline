    ...
    <xsl:template match="css:box[@type='block']">
        <xsl:copy>
            ...
            <xsl:choose>
                ...
                <xsl:when test="descendant::*[self::css:string | self::css:counter | self::css:text | self::css:content | self::css:leader | self::css:custom-func]">
                    <xsl:apply-templates select="node()" mode="remove-text-nodes"/>
                </xsl:when>
                
                <xsl:when test="matches(string(.), '^[\s&#x2800;]*$') and not(descendant::css:white-space)">
                    <xsl:apply-templates select="node()" mode="remove-text-nodes"/>
                </xsl:when>
                
                <xsl:otherwise>
                    <!--
                        Can a `css:box` with `@type='block'` occur inside another `css:box` with `@type='block'` be nested?
                        If not, we could do xsl:copy-of here instead:
                    -->
                    <xsl:apply-templates select="node()" mode="#current"/>
                </xsl:otherwise>
